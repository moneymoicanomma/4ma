...
import {
  ...
  type PressCredentialPublicResponse,
} from "@/lib/contracts/press-credential";
import { publicApiResponse } from "@/lib/server/api-response";
import { getServerEnv } from "@/lib/server/env";
import { submitPressCredential } from "@/lib/server/press-credential";
...
const successPayload: PressCredentialPublicResponse = {
  ok: true,
  message:
    "Cadastro recebido. A equipe vai revisar as informações e retornar pelo e-mail informado.",
};
...
export async function POST(request: NextRequest) {
  ...
  const rateLimit = takeRateLimitToken(`press-credential:${requester}`, {
    limit: 4,
    windowMs: 10 * 60 * 1000,
  });
  ...
  const parsed = parsePressCredentialSubmission(requestBody.data);
  ...
  const result = await submitPressCredential(parsed.data, requestContext, env);
